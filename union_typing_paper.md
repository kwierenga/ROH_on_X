...
A systematic search of the clinical and forensic literature (PubMed + web, 2025–2026) makes the data landscape explicit. Among *clinical* SNP-array/ROH reports, Sund 2013 is the **only** one that quantifies a female offspring's X autozygosity in a first-degree union — the rest report autosomal F and discard the X. The *forensic* literature does use the X, and supplies the closest thing to **ground-truth** female-offspring cases: a confirmed **father–daughter** incest (Dash et al. 2022, female fetus, X-STR homozygosity ≈53%, the daughter sharing the father's X) and a confirmed **brother–sister** incest (FSI:GSS 2015, female newborn, X-STR "decisive") — both qualitatively consistent with the F_X = ½ (FD) vs ¼ (SS) prediction, though at marker level rather than whole-X ROH. Two systematic gaps emerge: forensic X work treats the X as a binary detector and **omits mother–son entirely** (including the 2025 machine-learning treatment, which models only FD and SS), and across all sources — clinical, forensic, biobank, and ancient DNA — we found **no offspring genome from a confirmed mother–son union**. The sex-path *typing* this paper formalises is therefore essentially unobserved in real data, except incidentally (Sund's single female) and at the population scale (below).
...
1. **Single-genome union *typing* of first-degree unions from the X sex-path.** The ½-vs-¼ X-autozygosity contrast that separates parent–offspring from full-sib matings is *not* itself new — Yengo et al. (2019) used X-chromosome F_roh with the expectation ½ vs ¼ (observed mean 0.53 vs 0.34 across 26 female cases) to split these classes among extreme-inbreeding individuals in the UK Biobank — but that work, like the forensic literature, **collapses father–daughter and mother–son into one "parent–offspring" class**. What is unaddressed is resolving the **FD-vs-MS sex-path split** (the irreducible hard pair, which the population X-ratio cannot reach) and casting FD/MS/SS as a single-genome classification with an explicit, calibrated posterior.
...
- **The mother–son arm has no real-data anchor.** A systematic search found no published offspring genome from a confirmed mother–son union (clinical, forensic, biobank, or ancient DNA); the FD and SS arms have incidental/ground-truth female-X observations (Sund 2013; Dash 2022; FSI:GSS 2015) but MS does not. The ½ expected F_X for MS, and the FD-vs-MS results, are therefore validated only in simulation and theory.
...
20. Yengo L, Wray NR, Visscher PM. Extreme inbreeding in a European ancestry sample from the contemporary UK population. *Nat Commun* 2019;10:3719. doi:10.1038/s41467-019-11724-6 (PMID 31481654). [Verified in full text: in 26 female extreme-inbreeding cases, X-chromosome F_roh with expectation ½ (parent–offspring) vs ¼ (full-sib), observed mean 0.53 vs 0.34 — but father–daughter and mother–son are collapsed into one parent–offspring class.]
21. Dash HR, Gautam I, Singh AK, Shrivastava P. Genetic analysis of father–daughter incest using multifaceted STR markers and study of inheritance pattern of alleles. *Curr Forensic Sci* 2022;1:e020921195431. doi:10.2174/2666484401666210809114452.
